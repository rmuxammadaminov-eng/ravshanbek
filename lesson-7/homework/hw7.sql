...
select min(price) as pastnarh from Products 
...
(40, 'Gary', 'Wright', 'Marketing', 87000.00, '2021-01-10', 29, NULL, 'UK');
   ...
   select max(Salary) as boy from Employees
...
select count(*) as nulls from  Customers
SELECT COUNT(DISTINCT category) AS unique_categories_count
FROM Products;
CREATE TABLE Sales (
...
select sum(SaleAmount) as alls from Sales
WHERE ProductID= 7
select avg(age) as urtayosh from Employees
...
GROUP BY DepartmentName
HAVING COUNT(*) > 5;
select  ProductID ,sum(SaleAmount) as alls,
avg(SaleAmount) as urtacha  from Sales group by  ProductID
...
FROM Employees
GROUP BY DepartmentName;
select DepartmentName,avg(Salary) as boyvacha from Employees group by DepartmentName
SELECT DepartmentName,
